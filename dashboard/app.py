...
with tab1:
    ...
    if baseline_entry["error"] or hho_entry["error"]:
        ...
        ds = load_prepared_source(selected_source, N_INPUT, N_FORECAST)
        last_price = float(ds['data']['Close'].iloc[-1])
        last_date = ds['data'].index[-1]

        pred_baseline = forecast_next(baseline_entry["model"], ds)
        pred_hho = forecast_next(hho_entry["model"], ds)
        ...
        lookback = 90
        hist = ds['data']['Close'].iloc[-lookback:]
...
with tab2:
        ...
        for source in SOURCES:
            for arch in ARCHITECTURES:
                ...
                if entry["error"] is not None:
                    continue
                ds = load_prepared_source(source, N_INPUT, N_FORECAST)
                splits = predict_all_splits(entry["model"], ds)
    ...
    if not all_rows:
        ...
        df_metrics = df_metrics[["Sumber", "Model", "Split", "Horizon", "RMSE", "MAE", "MAPE (%)"]]

        st.markdown("**A. Tabel Perbandingan Lengkap (Test Set, per Horizon)**")
        df_test = df_metrics[df_metrics["Split"] == "TEST"]
        st.dataframe(
            df_test.style.format({"RMSE": "{:.4f}", "MAE": "{:.4f}", "MAPE (%)": "{:.2f}"}),
        ...
        st.dataframe(
            df_avg[["Sumber", "Model", "RMSE", "MAE", "MAPE (%)"]]
            .style.format({"RMSE": "{:.4f}", "MAE": "{:.4f}", "MAPE (%)": "{:.2f}"}),
            width='stretch', hide_index=True,
        )
